...
matrix = matrix[['stop_id_from','stop_id_to','movements_norm', 'hour_on']]
...
matrix = matrix[['super_site_x','super_site_y','movements_norm','hour_on']].rename(columns = {'super_site_x':'super_site_from',
                                                                                   'super_site_y':'super_site_to'})
matrix = matrix.groupby(['super_site_from','super_site_to','hour_on']).sum().reset_index()
...
links = links[['super_site_from','super_site_to','movements_norm','X_from','Y_from','X_to','Y_to','hour_on']]
...
links7 = links[links['hour_on'] == 7]
links8 = links[links['hour_on'] == 8]
...
Time_Title = Title(text='', text_font_size='8pt', text_color = 'grey')
p_to.add_layout(Time_Title, 'above')
...
menu = [('links7', 'links7'), ('links8', 'links8')]
...
layout1 = layout.row(p,p_to, select, stats)
